...
function _bsaPRO() {
    ...
    var writescript = null, scripts = 0;
    ...
    function flushWrite(which, data, incremental) {
        var id = 'write_' + which.id, writer = document.getElementById(id);
        ...
        scriptqueue.push({
            which: id, run: (function (id, sc) {
                return function () {
                    ...
                    if (sc.src)writescript.src = sc.src; else {
                        try {
    ...
    function isValidHtml(html) {
        var started = 0, smallstr = false, bigstr = false, tags = {}, tagnames = 'a|abbr|acronym|address|applet|article|aside|audio|b|bdi|bdo|big|blockquote|body|button|canvas|caption|center|cite|code|colgroup|command|datalist|dd|del|details|dfn|dialog|dir|div|dl|dt|em|embed|fieldset|figcaption|figure|font|footer|form|frameset|head|header|hgroup|h1|h2|h3|h4|h5|h6|html|i|iframe|ins|kbd|keygen|label|legend|li|map|mark|menu|meter|nav|noframes|noscript|object|ol|optgroup|output|p|pre|progress|q|rp|rt|ruby|s|samp|script|section|select|small|span|strike|strong|style|sub|summary|sup|table|tbody|td|textarea|tfoot|th|thead|time|title|tr|track|tt|u|ul|var|video'.split('|'), tagnames = ['script', 'div', 'span'], temp = null;
        for (var i = 0; i < html.length; i++) {
            ...
            if (ch == '"') {
                ...
                smallstr = !smallstr
            } else if (smallstr || bigstr) {
            } else if (ch == '<') {
                started++;
                temp = ''
            } else if (started > 0 && (ch == ' ' || ch == '>')) {
                if (temp !== null) {
                    var tag = temp, close = tag[0] == '/';
    ...
    window['_bsap_serving_callback'] = function (banner, zone, freqcap) {
        var append = function (w, data, days) {
            var c = document.cookie, i = c.indexOf(w + '='), existing = i >= 0 ? c.substring(i + w.length + 1).split(';')[0] + ',' : '', d = new Date();
    ...
    for (var zones = [], zonesegments = {}, cl = function (cl) {
        ...
        return ret
    }, bs = cl('bsaPROrocks'), segments = [], seg, zoneid, id, p = /bsap_([a-f0-9]+)/i, i = 0; i < bs.length && (zoneid = bs[i].getAttribute('data-serve')) && (bs[i].className = 'bsap'); i++) {
        ...
        for (var k = 0; k < bs[i].attributes.length; k++) {
            if (bs[i].attributes[k].name.substring(0, 12) == 'data-segment') {
                var segkey = bs[i].attributes[k].name.substring(13), vals = bs[i].attributes[k].value.split(';');
    ...
    var nostats = window.location.href.match(/bsaprostats=no/), force = window.location.href.match(/bsaproforce=([0-9]+)/), ignore = window.location.href.match(/bsaproignore=([0-9]+)/), ignoretarg = window.location.href.match(/bsaproignoretargeting/), maxp = window.location.href.match(/bsapromaxp=([0-9]+)/), preview = window.location.href.match(/preview=([A-Z0-9]+)/);
    ...
    if (force)url += '&forcebanner=' + force[1]; else if (window['_bsap_forcebanner'])url += '&forcebanner=' + window['_bsap_forcebanner'];
    if (ignore)url += '&ignorebanner=' + ignore[1]; else if (window['_bsap_ignorebanner'])url += '&ignorebanner=' + window['_bsap_ignorebanner'];
    if (ignoretarg)url += '&ignoretargeting=yes'; else if (window['_bsap_ignoretargeting'])url += '&ignorebanner=yes';
    if (maxp)url += '&maxpriority=' + maxp[1]; else if (window['_bsap_maxpriority'])url += '&maxpriority=' + window['_bsap_maxpriority'];
    if (nostats)url += '&ignore=yes';
    if (preview)segments.push([preview[1], [['SEGMENT', 'previewonly']]]);
    ...
    var ck = document.cookie, day = ck.indexOf('_bsap_daycap='), life = ck.indexOf('_bsap_lifecap=');
    ...
    if (day.length || life.length) {
        ...
        for (var i = 0; i < day.length; i++) {
            var adspot = day[i];
            for (var found = -1, find = 0; find < freqcap.length && found == -1; find++)if (freqcap[find][0] == adspot)found = find;
            if (found == -1)freqcap.push([adspot, 1, 0]); else freqcap[found][1]++
        }
        for (var i = 0; i < life.length; i++) {
            var adspot = day[i];
            for (var found = -1, find = 0; find < freqcap.length && found == -1; find++)if (freqcap[find][0] == adspot)found = find;
            if (found == -1)freqcap.push([adspot, 0, 1]); else freqcap[found][2]++
        }
        for (var i = 0; i < freqcap.length; i++)freqcap[i] = freqcap[i].join(',');
        if (freqcap.length)url += '&freqcap=' + freqcap.join(';')
    }
    c.src = url;
